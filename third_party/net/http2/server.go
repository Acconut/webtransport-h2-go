...
func (sc *serverConn) serve(conf http2Config) {
	...
	if wt := sc.srv.WebTransport; wt != nil {
		settings = append(settings, wt.settings()...)
	}
...
func (sc *serverConn) newWriterAndRequestNoBody(st *stream, rp httpcommon.ServerRequestParam) (*responseWriter, *http.Request, error) {
	...
	req := (&http.Request{
		...
		Body:       body,
		Trailer:    res.Trailer,
	}).WithContext(context.WithValue(st.ctx, PeerWebTransportSettingsContextKey, sc.peerWebTransport.snapshot()))
	rw := sc.newResponseWriter(st, req)
	return rw, req, nil
}
