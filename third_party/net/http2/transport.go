...
type Transport struct {
	...
	// WebTransport, if non-nil, causes WebTransport-over-HTTP/2 SETTINGS
	// (draft-ietf-webtrans-http2-15) to be included in the initial SETTINGS
	// frame. See DefaultWebTransportSettings.
	// Clients typically set Enabled=false (support is signaled via CONNECT)
	// but should advertise non-zero initial flow-control limits.
	// webtransport-h2-go
	WebTransport *WebTransportSettings
...
type ClientConn struct {
	...
	strictMaxConcurrentStreams  bool
	peerWebTransport            peerWebTransportSettings // webtransport-h2-go
...
func (t *Transport) newClientConn(c net.Conn, singleUse bool, internalStateHook func()) (*ClientConn, error) {
	...
	// webtransport-h2-go: advertise WebTransport SETTINGS (draft-15).
	if wt := t.WebTransport; wt != nil {
		initialSettings = append(initialSettings, wt.settings()...)
	}
...
// PeerWebTransportSettings returns the peer's most recently received
// WebTransport-over-HTTP/2 SETTINGS (draft-ietf-webtrans-http2-15).
// Omitted parameters default to 0. webtransport-h2-go
func (cc *ClientConn) PeerWebTransportSettings() WebTransportSettings {
	return cc.peerWebTransport.snapshot()
}
...
func (rl *clientConnReadLoop) processSettingsNoWrite(f *SettingsFrame) error {
	...
	err := f.ForeachSetting(func(s Setting) error {
		...
		case SettingWTEnabled,
			SettingWTInitialMaxData,
			SettingWTInitialMaxStreamDataUni,
			SettingWTInitialMaxStreamDataBidiLocal,
			SettingWTInitialMaxStreamDataBidiRemote,
			SettingWTInitialMaxStreamsUni,
			SettingWTInitialMaxStreamsBidi:
			// webtransport-h2-go: record peer WebTransport SETTINGS.
			if s.ID == SettingWTEnabled {
				if err := s.Valid(); err != nil {
					return err
				}
			}
			cc.peerWebTransport.apply(s)
		default:
